...
SHIFT <- FALSE
if (SHIFT) {
  Data$GGVEW <- ShiftInTime (Data$GGVEW, Rate, s)    ## this is needed for pitch and heading corrections
...
SV <- with(Data[1, ], data.frame(LAT, LON, ALT, VEW, VNS, VSPD, PITCHL, ROLLL, THDG,
                                 BPITCHR, BROLLR, BYAWR, BLATA, BLONGA, BNORMA))
SV[c(1,2,7:12)] <- SV[c(1,2,7:12)] * Cradeg
NSTEP <- 5      ## update time
