...
<<Kalman-setup, include=TRUE, echo=FALSE, cache=TRUE>>=
...
GCF <- function (sv, sp) {    ## sv is the state vector; sp is the data record
  ...
  gcf[3] <- 150
  gcf[4:6] <- 0.5
  gcf[7:8] <- 0.005*Cradeg
  gcf[9] <- 0.015*Cradeg
  gcf[10:11] <- 0.015*Cradeg*sqrt(2/tau)
  gcf[12] <- 0.015*Cradeg*sqrt(2/tau)
  gcf[13:15] <- 0.001*sqrt(2/tau)
  return (gcf)
}
...
D1$deltaPsi <- (atan2 (D1$LACCY, D1$LACCX) - atan2 (D1$vedot, D1$vndot)) / Cradeg
...
RCV[1,1] <- (30/D1$Rm[1])^2  ## latitude
RCV[2,2] <- (30/(D1$Rn[1]*cos(SV[1])))^2
RCV[3,3] <- 50^2
RCV[4,4] <- 3^2            ## ve
RCV[5,5] <- 3^2
RCV[6,6] <- 0.3^2
...
#   SVEL[8] <- sin(sv[9]) * SVEA[7] + cos(sv[9]) * SVEA[8]
  ...
  CV <- dcm %*% (CV %*% t(dcm)) + Q
  ## the Kalman gain:
  # if (is.na(Data$sdPsi[i])) {
  #   RCV[10,10] <- 225
  # } else {
  #   RCV[10,10] <- Data$sdPsi[i]^2
  # }
