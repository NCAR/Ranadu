...
Cradeg <- pi/180
# remove 'cal coef' in data archive
Data$THDG <- (Data$THDG + 0.08) %% 360
.shift = 60  # previously shifted backward 60 ms in processing
Data$THDG <- ShiftInTime (Data$THDG, 25, .shift)
Data$PITCH <- ShiftInTime (Data$PITCH, 25, .shift)
Data$ROLL <- ShiftInTime (Data$ROLL, 25, .shift)
.shift <- 0
for (V in c('BPITCHR', 'BROLLR', 'BYAWR')) {
...
DL <- nrow(Data)
omegaE <- StandardConstant ('Omega') #* 0.95  ### NOTE!!!
# omegaE <- 2*pi/86400
Ree <- 6378137
Ecc <- 0.08181919
...
STMFV <- function (sv, .aaframe='a') { 
  ...
  Oill <- matrix (c(0, omega[3], omega[2], -omega[3], 0, -omega[1], 
  ...
  A <- 0.10
  A <- 0
  YC <- 1-A*(1-2*sin(sv[9]-5*Cradeg)^2)
  RRT[3] <- YC * RRT[3]
  stmf <- c(DR, AL, RRT,
           # -dRLA[3,1]/sqrt(1-rlm[3,1]^2), 
           # 1/(1+(rlm[3,2]/rlm[3,3])^2) * (rlm[3,2]/rlm[3,3]) * 
           #   (-dRLA[3,2]/rlm[3,2] + dRLA[3,3]/rlm[3,3]), 
           # 1/(1+(rlm[1,1]/rlm[1,2])^2) * (rlm[1,1]/rlm[1,2]) * 
           #   (dRLA[1,1]/rlm[1,1] - dRLA[1,2]/rlm[1,2]) * YC,
            0, 0, 0, 0, 0, 0)
...
SP$BNORMA <- SP$BNORMA*1.005 - 0.025
...
op <- par (mar=c(2,4,1,1)+0.1)
layout(matrix(1:3, ncol = 1), widths = 1,  heights = c(5,5,6))
plotWAC (Data[r, c('Time', 'PITCH', 'PITCHX', 'DPITCH')], legend.position='right')
...
\section{A new rate-of-climb variable}

The preceding highlights the weakness of using VSPD from the INS as
a measurement of rate of climb of the aircraft, primarily because
it is updated to pressure altitude instead of geometric altitude.
...
component, so it is useful to consider another variable based on the
INS measurements that can then be updated to GGVSPD via the Kalman
filter. The hydrostatic equation provides a basis for updating that
is independent of the GPS: 
...
VarList <- c(VarList, 'TASX', 'ATTACK', 'SSLIP', 'PSXC', 'ATX', 'ACINS')
...
if (SHIFT) {
  ...
  D1$GGVSPD <- ShiftInTime (D1$GGVSPD, Rate, s)
...
tau <- 300
D1$DIFW <- signal::filtfilt (signal::butter (3, 2/tau), D1$DIF)
...
<<ROC-plot, include=TRUE, fig.cap='Comparison of new variable representing rate of climb (ROC) to the GPS-provided measurement (GGVSPD) and the measurement provided by the INS (VSPD).'>>=

layout(matrix(1:3, ncol = 1), widths = 1,  heights = c(5,5,6)) 
op <- par (mar=c(2,4,1,1)+0.1)
with(D1[setRange(D1, 70000, 122000), ], plotWAC (data.frame (Time, ROC, GGVSPD), 
                                                 col=c('blue', 'red'), lwd=c(3,1), lty=c(1,2)))
with(D1[setRange(D1, 70000, 122000), ], plotWAC (data.frame (Time, ROC-GGVSPD)))
op <- par (mar=c(5,4,1,1)+0.1)
with(D1[setRange(D1, 75500, 75900), ], plotWAC (data.frame (Time, ROC, GGVSPD, VSPD), ylim=c(-2,2))) 
op <- par (mfrow=c(1,1), mar=c(5,5,2,2)+0.1)  # reset
...
DZ <- with(D1, c(LAT-GGLAT, LON-GGLON, ALT-GGALT, VEW-GGVEW + LG*Hdot*cospsi, 
                 VNS-GGVNS - LG*Hdot*sinpsi, ROC-GGVSPD + LG*Pdot))
...
the accelerations from the INS and so could have the best high-frequency
...
.hdg <- Data$THDGKF * Cradeg
.wd <- Data$WDKF * Cradeg + pi
Data$UXKF <- Data$WSKF * (sin(.hdg)*sin(.wd) + cos(.hdg)*cos(.wd))
.hdg <- .hdg - pi/2
Data$VYKF <- Data$WSKF * (sin(.hdg)*sin(.wd) + cos(.hdg)*cos(.wd))
DataW$GGVSPD <- Data$GGVSPD
DataN <- WindProcessor(DataW, CompF=FALSE)    ## suppress comp filter and GPS lever arm)
Data$WIKFG <- DataN$WIN
sdWIdif <- sd(Data$WIKF-Data$WICC, na.rm=TRUE)
...
figure is \Sexpr{round(sdWIdif, 2)}~m/s, so the difference is significant
in comparison to the claimed uncertainty in measuring vertical wind. 
...
<<plot-WIC, include=TRUE, fig.cap='Vertical wind calculated using the results from the Kalman filter (WIKF) and the original calculation. The difference is shown in the bottom panel. WIKF is calculated using ROCKF and other results after correction by the Kalman filter, while WIC was calculated using GGVSPD from the GPS for the rate-of-climb of the aircraft. Data from a segment of DEEPWAVE flight 16 where strong waves were encountered.'>>=

layout(matrix(1:2, ncol = 1), widths = 1,  heights = c(5,6))
op <- par (mar=c(2,4,1,1)+0.1)
with(Data[setRange(Data, 83400, 83800),], plotWAC (data.frame(Time, WIKF, WICC),
                                                   legend.position=NA))
legend ('bottomleft', legend=c('WIKF', 'WIC'), col=c('blue', 'darkgreen'), lwd=c(2,1), cex=0.8)
op <- par (mar=c(5,4,1,1)+0.1)
with(Data[setRange(Data, 83400, 83800),], plotWAC (data.frame(Time, WIKF-WICC), ylab='WIKF-WIC'))
op <- par (mfrow=c(1,1), mar=c(5,5,2,2)+0.1)  # reset
...
layout(matrix(1:2, ncol = 1), widths = 1,  heights = c(5,6))
op <- par (mar=c(2,4,1,1)+0.1)
with(Data[r,], plotWAC (data.frame(Time, VEWKF-GGVEW)))
op <- par (mar=c(5,4,1,1)+0.1)
with(Data[r,], plotWAC (data.frame(Time, VNSKF-GGVNS)))
op <- par (mfrow=c(1,1), mar=c(5,5,2,2)+0.1)  # reset

@

<<hw-plot, include=TRUE, fig.cap='Differences between the conventional measurements of horizontal wind (WDC and WSC) and the results from the Kalman filter (WDKF and WSKF).'>>=

layout(matrix(1:2, ncol = 1), widths = 1,  heights = c(5,6))
op <- par (mar=c(2,4,1,1)+0.1)
with(Data[r,], plotWAC (data.frame(Time, WDKF-WDCC), ylab='WDKF-WDC'))
op <- par (mar=c(5,4,1,1)+0.1)
with(Data[r,], plotWAC (data.frame(Time, WSKF-WSCC), ylab='WSKF-WSC'))
op <- par (mfrow=c(1,1), mar=c(5,5,2,2)+0.1)  # reset
...
The code in this section adds those variables to an existing newCDF
file, renaming the result with names like PROJECTrf01KF.nc if the
original file was PROJECTrf01.nc. For reference, the following is
a generic description of how variables are added. The listed code
is not executed; see the ``.Rnw'' file for the actual code, which
has more steps to protect against missing variables or substitute
variables when some are missing and to handle high-rate data files.
This example is included to serve as a template for other routines
that might want to add variables to a netCDF file.

<<example, eval=FALSE, echo=TRUE, include=TRUE>>=
...
VarNew <- c('LATKF')    ## add to these vectors for more variables
VarOld <- c('LAT')
VarUnits <- c('degrees')
...
nv <- length (VarNew)
varCDF <- list ()
for (i in 1:nv) {  ## there is only one in this example
  ...
  ## transfer attributes from the old variable
...
VarList <- c('TASX', 'ATTACK', 'SSLIP', 'GGVEW', 'GGVNS', 'GGVSPD', 'VEW', 'VNS', 'THDG', 'ROLL', 'PITCH',
             'LAT', 'LON', 'VSPD')
VarListRef <- VarList
...
VarOld <- c('LAT', 'LON', 'ALT', 'VEW', 'VNS', 'VSPD', 'PITCH', 'ROLL', 'THDG', 'WD', 'WS', 'WI', 'WIC', 'WIC', 'WDC', 'WSC', 'UXC', 'VYC')
...
\section{Material for the Appendix}
...
accelerations of the a-frame relative to an inertial frame. For early
projects these measurements were not recorded, and for most projects
they have not been retained in the archived files. This appendix describes
how they can be retrieved from the measurements of attitude angles
and the aircraft velocity vector. Without Coriolis corrections, the
answers would be straightforward: Differentiate the measured angles
and components of the aircraft velocity and transform the results
to the a-frame. However, the inertial contributions from motion and
rotation of the a-frame, while minor, are not negligible. The needed
corrections are developed in this Appendix.

\subsection{Accelerations}

\subsection{Rotation rates}

The equations used for mechanization were developed in this report
and in the workflow description that accompanies it. In the case of
the attitude angles, the derivatives of the attitude angles were obtained
by expressing those angles in terms of appropriate components of the
rotation matrix, for example by finding the time derivative of pitch
from \\
\[
\dot{\theta}=d(\frac{\mathrm{d(}\arcsin(-R_{a}^{l}[3,1])\mathrm{)}}{\mathrm{d}t}=\frac{-1}{\sqrt{1-R_{a}^{l}[3,1]^{2}}}\dot{R}_{a}^{l}[3,1]
\]
where $\boldsymbol{R}_{a}^{l}$ is the transformation matrix from
the a-frame to the l-frame, which has component {[}3,1{]} equal to
$-\sin\theta$. The derivative of the transformation matrix $\boldsymbol{\dot{R}_{a}^{l}}$
is found by transforming the measured body rotation rates to the l-frame
and then adding the inertial correction term $-\Omega_{il}^{a}\boldsymbol{R}_{a}^{l}$
where, if $\omega$ is defined as\\
\[
...
\end{array}\right]
\]
with $\Omega_{E}$ the angular rotation rate of the Earth, the skew-symmetric
representation of $\omega$ is $\Omega_{il}^{l}$: \\
\[
...
of the transformation matrix, now expressed in terms of the l-frame
angular rates represented by \{$\dot{\theta}$, $\dot{\phi}$, $\dot{\psi}$\},
which can be determined by differentiating the measurements of pitch,
roll, and heading. This is $\dot{\boldsymbol{R}}_{a}^{l}$, and its
components in terms of the derivatives of the measured angles are
as follows:
\begin{quotation}
{[}1,1{]}: dR{[}1,1{]}dt = d($\sin\psi\cos\theta$)/dt = $\dot{\psi}\cos\psi\cos\theta-\dot{\theta}\sin\psi\sin\theta$

{[}1,2{]}: $\sin\psi\sin\theta\sin\phi+\cos\psi\cos\phi\rightarrow\dot{\psi}(\cos\psi\sin\theta\sin\phi-\sin\psi\cos\phi)+\dot{\theta}\sin\psi\cos\theta\sin\phi+\dot{\phi}(\sin\psi\sin\theta\cos\phi-\cos\psi\sin\phi)$

{[}1,3{]}: $\cos\psi\sin\phi-\sin\psi\sin\theta\cos\phi\rightarrow\dot{\psi}(-\sin\psi\sin\phi-\cos\psi\sin\theta\cos\phi)+\dot{\theta(-}\sin\psi\cos\theta\cos\phi)+\dot{\phi}(\cos\psi\cos\phi+\sin\psi\sin\theta\sin\phi)$

{[}2,1{]}: $\cos\psi\cos\theta\rightarrow\dot{\psi}(-\sin\psi\cos\theta)+\dot{\theta}(-\cos\psi\sin\theta)$

{[}2.2{]}: $\cos\psi\sin\theta\sin\phi-\sin\psi\cos\phi\rightarrow\dot{\psi}(-\sin\psi\sin\theta\sin\phi-\cos\psi\cos\phi)+\dot{\theta}\cos\psi\cos\theta\sin\phi+\dot{\phi}(\cos\psi\sin\theta\cos\phi+\sin\psi\sin\phi)$

{[}2.3{]}: $-\cos\psi\sin\theta\cos\phi-\sin\psi\sin\phi\rightarrow\dot{\psi}(\sin\psi\sin\theta\cos\phi-\cos\psi\sin\phi)+\dot{\theta}(-\cos\psi\cos\theta\cos\phi)+\dot{\phi}(\cos\psi\sin\theta\sin\phi-\sin\psi\cos\phi)$

{[}3,1{]}: $-\sin\theta\rightarrow\dot{\theta}(-\cos\theta)$

{[}3,2{]}: $\cos\theta\sin\phi\rightarrow\dot{\theta}(-\sin\theta\sin\phi)+\dot{\phi}\cos\theta\cos\phi$

{[}3,3{]}: $-\cos\theta\cos\phi\rightarrow\dot{\theta}\sin\theta\cos\phi+\dot{\phi}\cos\theta\sin\phi$
\end{quotation}
Add to this the correction for inertial effects (i.e., $+\boldsymbol{\Omega}_{il}^{a}\boldsymbol{R}_{a}^{l}$),
then transform back to the a-frame using $\boldsymbol{R}_{l}^{a}$
(i.e., the inverse of $\boldsymbol{R}_{a}^{l}$) to get the skew-symmetric
representation of the rotation rates as measured in the a-frame. Then,
for example, the {[}3,1{]} component of the result is $-$BPITCHR.

<<recover-rotation-rates, include=TRUE, fig.cap=''>>=
...
D1$BR <- OmegaA[,2,3]
D1$BY <- -OmegaA[,1,2]
with(D1[setRange(D1, 80000,83000),], plotWAC(data.frame(Time, BY, BYAWR)))
with(D1[setRange(D1, 80000,83000),], plotWAC(data.frame(Time, BP, BPITCHR)))
with(D1[setRange(D1, 80000,83000),], plotWAC(data.frame(Time, BR, BROLLR)))
