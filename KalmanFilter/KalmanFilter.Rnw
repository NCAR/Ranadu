...
the accelerations from the INS and so might have the best high-frequency
performance, but GPS receivers provide such good measurements now
that it may be preferable to use GGVSPD directly. The other difference
between these two choices is that if GGVSPD is used then a correction
is needed for the about-6~m displacement of the GPS antenna from
the INS. This needs a lever-arm correction, while ROCKF is based on
measurements from the INS and so does not need such a correction.
...
VarNew <- c('LATKF', 'LONKF', 'ALTKF', 'VEWKF', 'VNSKF', 'ROCKF', 'PITCHKF', 'ROLLKF', 'THDGKF',
            'WDKF', 'WSKF', 'WIKF', 'WIKFG')
VarOld <- c('LAT', 'LON', 'ALT', 'VEW', 'VNS', 'VSPD', 'PITCH', 'ROLL', 'THDG', 'WD', 'WS', 'WI', 'WIC')
...
VarLongName <- c('latitude, KF', 'longitude, KF', 'altitude MSL, KF',
                 ...
                 'wind direction, KF', 'wind speed, KF', 'vertical wind, KF', 'vertical wind, KF and GGVSPD')
VarStdName <- c('INS latitude, Kalman-filter-corrected',
