...
GCF <- function (sv, sp) {
  ...
  gcf[4:5] <- 0.15
  gcf[6] <- 1.5
  gcf[7:9] <- 0.03*Cradeg  #0.05*Cradeg  ## large error gives better updating
...
Data$HC <- CorrectHeading (Data, .plotfile='./HCPlot.pdf') + 0.08
...
abline(h=0, col='darkorange', lty=2)
plotWAC(subset(Data[r, ],, c(Time, CTHDG, HC)), ylab=expression(paste('heading correction [',degree,']')), ylim=c(-0.1,0.1), lwd=0.7)
HE <- VCor[,9]
HE[HE < 0.0025] <- 0.0025
HE <- sqrt(HE)
...
SS <- smooth.spline(Data$Time[r], Data$CTHDG[r], w=1/HE[r]^2, spar=1)
...
plotWAC (subset(Data[r,],,c(Time, pitch_L, roll_L)), ylim=c(-0.05,0.05), ylab=expression (paste ('correction [',degree,']')), main="l-frame corrections")
